...
class DDPMSampler:
    ...
    def set_strength(self, strength=1):
        """
        if we consider an image as our condition, in this function we add noise to it
        """
    ...
    def step(self, timestep: int, latents: torch.Tensor, model_output: torch.Tensor):
        """
        remove predicted mean of the noise from the input to prepare the input of the next time step
        """
        t = timestep
    ...
    def add_noise(self, original_samples: torch.FloatTensor, timesteps: torch.IntTensor) -> torch.FloatTensor:
        """
        forward process (add noise to the input image to generate noisy images at each time step)
        """
